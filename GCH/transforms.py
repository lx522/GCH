...
def drop_feature(x, drop_prob):# x(76510,745) drop_prob(745,)
...
def drop_feature_weighted_2(x, w, p: float, threshold: float = 0.7):
    w = w / w.mean() * p
    w = w.where(w < threshold, torch.ones_like(w) * threshold)
    drop_prob = w# 最终的维度i被屏蔽的概率pi^f

    drop_mask = torch.bernoulli(drop_prob).to(torch.bool)# 屏蔽向量m
...
def feature_drop_weights(x, node_c):# x（7650，745）node_c（7650，）
...
def drop_edge_weighted(edge_index, edge_weights, p: float, threshold: float = 1.):# p=0.2 threshold=0.7 edge_index=（2，253536）edge_weights=（253536，）
    edge_weights = edge_weights / edge_weights.mean() * p
    edge_weights = edge_weights.where(edge_weights < threshold, torch.ones_like(edge_weights) * threshold)# 最终的边被删除的概率puv^e
    sel_mask = torch.bernoulli(1. - edge_weights).to(torch.bool)# 屏蔽矩阵R（253536，）
# sel_mask（253536，）: tensor([ True, True, True, ..., True, True,False]
    return edge_index[:, sel_mask]# edge_index=（2，253536）处理后的邻接矩阵A


def degree_drop_weights(edge_index):# edge_index(2,253536)
    edge_index_ = to_undirected(edge_index)# edge_index_(2,261230)
    deg = degree(edge_index_[1])# deg(7650,)
    deg_col = deg[edge_index[1]].to(torch.float32)# deg_col（2，253536）
    s_col = torch.log(deg_col)# 边重要性s_col(253536，)
    weights = (s_col.max() - s_col) / (s_col.max() - s_col.mean())# 系数weights（253536，）

    return weights# （253536，）
