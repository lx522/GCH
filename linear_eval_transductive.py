...
def main(argv):
    ...
    if FLAGS.dataset != 'wiki-cs':
        dataset = get_dataset(FLAGS.dataset_dir, FLAGS.dataset)# dataset=AmazonPhoto()
    else:
    ...
    if FLAGS.dataset != 'wiki-cs':
        score = fit_logistic_regression(representations.cpu().numpy(),labels.cpu().numpy(), "ps")[0]
    else:
        scores = fit_logistic_regression_preset_splits(representations.cpu().numpy(), labels.cpu().numpy(),
                                                       train_masks, val_masks, test_masks)
        score = np.mean(scores)

    print('Test score: %.5f' %score)# [0.9336601307189543] Test score: 0.93366
